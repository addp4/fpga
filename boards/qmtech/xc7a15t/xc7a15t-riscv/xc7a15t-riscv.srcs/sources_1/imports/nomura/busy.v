...
module busybeaver_143space(input clk,
   ...
   // Single-ported RAM
   reg [2:0]                    tape[(1 << MEMBITS)-1:0];  // size is power of 2 so pos can wrap
   wire [2:0]                   sym;
   reg [2:0]                    newsym;
...
module busybeaver(input clk,  // 50 MHz system clock
            input rst_n,
            output led,
            output din0,
            output ce_0,
   ...
   // clk_wiz_0 bbclk(.clk_in1(clk), .clk_out1(CLOCK_300), .clk_out2(CLOCK_250), .clk_out3(CLOCK_150));

   // Busybeaver module
   wire [63:0] bb_count[3:0];
   reg [63:0]  display_value;
   wire        bb0_halt;
   reg [31:0]  cnt;
   
   busybeaver_143space bb0(.clk(clk), .rst_n(rst_n), .count(bb_count[0]), .halt(bb0_halt));
   ...
   // led flashes while machine 0 is not halted
   assign led = bb0_halt ? 0 : cnt[24];
